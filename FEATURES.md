...
## Reconstruction status (updated 2026-06-10)

Since the original audit, **13 previously-broken flags have been reconstructed**
(missing source files rebuilt to their exact call-site contracts, verified
per-flag, and shipped). The `dev-full` build now compiles **49 flags**.

**Reconstructed & shipped in `cli-dev`:**

- Wave 1: `AUTO_THEME`, `BG_SESSIONS`, `BUDDY`, `COMMIT_ATTRIBUTION`,
  `CONTEXT_COLLAPSE`, `FORK_SUBAGENT`, `MCP_SKILLS`, `MONITOR_TOOL`
- Wave 2: `HISTORY_SNIP`, `REACTIVE_COMPACT`, `RUN_SKILL_GENERATOR`,
  `TEMPLATES`, `WEB_BROWSER_TOOL`

**Reconstructable but intentionally not shipped on-by-default:**

- `MEMORY_SHAPE_TELEMETRY` (telemetry — against the no-telemetry posture),
  `OVERFLOW_TEST_TOOL` (test-only), `TRANSCRIPT_CLASSIFIER` (auto-approve
  permission classifier — a safety boundary; opt-in).
- `DIRECT_CONNECT` — needs the full `claude server` subsystem; `parseConnectUrl.ts`
  groundwork is in place.

The per-flag sections below remain the detailed audit; treat the lists above as
the current shipped/deferred truth.
