...
export function isAnthropicAuthEnabled(): boolean {
  ...
  const hasStoredClaudeAiOAuth = shouldUseClaudeAIAuth(
    getClaudeAIOAuthTokens()?.scopes,
  )

  // Disable Anthropic auth if:
  // 1. Using 3rd party services (Bedrock/Vertex/Foundry)
  // 2. User has an external API key and no Claude.ai OAuth session
  ...
  //
  // Bun auto-loads repo-local .env files for direct ./cli-dev runs. If that
  // .env contains an Anthropic-compatible gateway key, it should not silently
  // override a real /login Claude subscription. Dedicated gateway launchers use
  // isolated CLAUDE_CONFIG_DIR values and still take the external-key path.
  const shouldDisableAuth =
    is3P ||
    (hasExternalAuthToken && !isManagedOAuthContext()) ||
    (hasExternalApiKey && !hasStoredClaudeAiOAuth && !isManagedOAuthContext())
...
async function checkAndRefreshOAuthTokenIfNeededImpl(
  ...
  const forcedAccessToken = force ? tokens.accessToken : undefined
  ...
  if (
    force &&
    freshTokens.accessToken !== forcedAccessToken &&
  ...
  try {
    ...
    if (
      force &&
      lockedTokens.accessToken !== forcedAccessToken &&
    ...
    clearKeychainCache()
    const currentTokens = await getClaudeAIOAuthTokensAsync()
    const recoveredFromConcurrentRefresh =
      currentTokens &&
      !isOAuthTokenExpired(currentTokens.expiresAt) &&
      (!force || currentTokens.accessToken !== forcedAccessToken)

    if (recoveredFromConcurrentRefresh) {
