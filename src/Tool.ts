...
export type ToolUseContext = {
  ...
  /** Opaque ID binding implementation workers to the active verification turn. */
  verificationContractId?: string;
  /** Prevent verifier-side test artifacts from being recorded as implementation mutations. */
  verificationRole?: VerificationRole;
  /** Filesystem and subprocess containment applied only to the trusted verifier. */
  verificationIsolation?: VerificationIsolation;
  /** Revision-bound mutation sinks installed by the verification contract.
   * Tools call these after successful writes instead of importing the contract
   * service, which keeps the tool layer independent of query orchestration. */
  recordVerificationFileMutation?: (filePath: string) => void;
  recordVerificationOpaqueMutation?: (reason: string) => void;
  /** Suppress user, project, plugin, and session hooks inside a harness-owned
   * trusted verifier. The verifier still uses internal harness enforcement. */
