...
export function getCodexTokenScopes(accessToken: string): string[] {
  const payload = decodeJwtPayload(accessToken)
...
export function hasCodexScope(accessToken: string): boolean {
  const scopes = new Set(getCodexTokenScopes(accessToken))
  return CODEX_REQUIRED_SCOPES.every(scope => scopes.has(scope))
}

function assertCodexScope(accessToken: string): void {
  if (!hasCodexScope(accessToken)) {
    throw new Error(
...
export async function exchangeCodexCode(
  ...
  assertCodexScope(result.access)
