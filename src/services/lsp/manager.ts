...
/**
 * The main tool pool is assembled before the post-trust LSP startup begins.
 * Harness sessions therefore keep the tool visible while initialization has
 * not started or is pending; LSPTool.call() waits for the pending promise.
 */
export function shouldExposeLspWhileInitializing(
  state: InitializationState,
  harnessEnabled: boolean
): boolean {
  return harnessEnabled && (state === "not-started" || state === "pending");
}
