...
export function shouldNotifyAutoCompactCircuitBreaker(
  consecutiveFailures: number,
...
export async function autoCompactIfNeeded(
  ...
  try {
    ...
    const harnessEnabled = isCodingHarnessEnabled();
    ...
    if (shouldNotifyAutoCompactCircuitBreaker(nextFailures, harnessEnabled)) {
      toolUseContext.addNotification?.({
        key: `autocompact-circuit-breaker-${toolUseContext.queryTracking?.chainId ?? "active"}`,
        text: "Automatic compaction failed repeatedly and has been disabled for this query chain.",
        priority: "high",
        timeoutMs: 12_000,
      });
    }
